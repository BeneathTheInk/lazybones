 ...
 * # Lazybones
 *
 * Lazybones is a JavaScript class that extends `Backbone.Collection` and uses [PouchDB](http://pouchdb.com) to persist data. A deeper understanding of [Backbone](http://backbonejs.org) is recommended since many of Lazybones methods and concepts are inherited directly from Backbone Collection. Knowledge of PouchDB's API is also recommended, but not required to use Lazybones.
 */
...
_.extend(Lazybones.prototype, {
	 ...
	 *   - **options.attachments** _boolean_ - An object of options to initiate the database with. This variable is passed directly to the `Backbone.Collection` constructor. Default value is `false`.
	 *   - **options.conflicts** _boolean_ - Whether or not to include conflicting revision ids in the `_conflicts` field. Default value is `true`.
	 ...
	 * ### destroy()
	 *
	 * Destroys the PouchDB underlying database. This will keep all the existing documents in-memory, so call `db.reset()` to remove those as well. The Lazybones instance is unusable once this method has been called.
	 *
	 ...
	 * ### _flushWrites()
	 *
	 * A private method that pushes the first 100 requests in the write queue to the database. When finished, this calls `db._invalidateWrites()` to continue the cycle.
	 *
	 * This method returns a promise that is resolved when the first 100 writes are completed.
	 */
